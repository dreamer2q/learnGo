...
func (m *MainScene) Widget() {
	...
	// draw image button
	if e := gui.ImageButton(6, gui.Rect{50, 30, 30, 30}, m.normal, m.pressed, nil); e&gui.EventWentDown != 0 {
		log.Println("Click Image Button", e)
	}
	...
	// draw slider
	if e := gui.Slider(7, gui.Rect{100, 0, 120, 10}, &m.slide, nil); e&gui.EventDragging != 0 {
		fmt.Println("slide value", m.slide)
	}
